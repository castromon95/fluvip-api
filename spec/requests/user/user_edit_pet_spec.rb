...
RSpec.describe 'Add Pet request', type: :request do
  ...
  context 'when jwt_token is invalid' do
    before do
      put (url + pet.id.to_s), headers: headers
    end
  ...
  context 'when admin user tries to access the service' do
    before do
      put (url + pet.id.to_s), headers: auth_header_admin
    end
  ...
  context 'when fields are fine' do
    before do
      put (url + pet.id.to_s), params: params, headers: auth_header_user1
    end
  ...
  context 'when fields are not fine' do
    before do
      put (url + pet.id.to_s), params: bad_params, headers: auth_header_user1
    end
  ...
  context 'when a user tries to edit another user pet' do
    before do
      put (url + pet.id.to_s), params: params, headers: auth_header_user2
    end
