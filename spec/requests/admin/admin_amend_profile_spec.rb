...
RSpec.describe 'Amend Profile request', type: :request do
  let(:profile) { Fabricate(:profile) }
  let(:profile2) { Fabricate(:profile) }
  let(:admin_profile) { Fabricate(:profile, user: Fabricate(:user, admin: true)) }
  let(:url) { '/profiles/amend/'}
  let(:headers) {{ 'Accept' => 'application/json' }}
  let(:auth_header_user1) {Devise::JWT::TestHelpers.auth_headers(headers, profile.user)}
  let(:auth_header_user2) {Devise::JWT::TestHelpers.auth_headers(headers, profile2.user)}
  ...
  context 'when jwt_token is invalid' do
    before do
      post (url + profile.id.to_s), params: params, headers: headers
    end
  ...
  context 'when no admin user tries to access the service to edit another persona profile' do
    before do
      post (url + profile.id.to_s), params: params, headers: auth_header_user2
    end
  ...
  context 'when fields are fine' do
    before do
      post (url + profile.id.to_s), params: params, headers: auth_header_admin
    end
  ...
  context 'when fields are not fine' do
    before do
      post (url + profile.id.to_s), params: bad_params, headers: auth_header_admin
    end
  ...
  context 'when no admin user tries to access the service to edit their own profile' do
    before do
      post (url + profile.id.to_s), params: params, headers: auth_header_user2
    end
